import { convertAtomicDecimals } from "../amounts";
...
export async function getPoolProtocolQuote(
  ...
  url.searchParams.set("destination", request.recipient);
  ...
  try {
    ...
    if (!response.ok || typeof rawResponse.expected_amount_out !== "string") {
      return { protocol, strategy, status: "error", requestStartedAt, responseReceivedAt, errorCode: `HTTP_${response.status}`, errorMessage: String(rawResponse.message ?? "Quote unavailable"), rawResponse };
    }
